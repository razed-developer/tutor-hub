(() => {
  ...
  const table=$('multiplication-table'), input=$('equation'), result=$('result'), drawer=$('drawer'), equationDrawer=$('equationDrawer');
  let size=12;
  ...
  function tap(td){if(state.practice){td.classList.toggle('revealed');result.textContent=td.classList.contains('revealed')?`${td.dataset.row} × ${td.dataset.col} = ${td.dataset.value}`:'Answer hidden again.'}else{input.value=`${td.dataset.row}×${td.dataset.col}=`;evaluate();setEquationDrawer(true);input.focus()}}
  ...
  function applyModes(){table.classList.toggle('practice',state.practice);table.querySelectorAll('td').forEach(td=>{const r=+td.dataset.row,v=+td.dataset.value,c=+td.dataset.col;td.classList.toggle('prime',state.primes&&isPrime(v));td.classList.toggle('square',state.squares&&r===c);td.classList.toggle('symmetry',state.symmetry&&r!==c);td.classList.toggle('symmetry-line',state.symmetry&&r===c);td.classList.toggle('even',state.parity&&v%2===0);td.classList.toggle('odd',state.parity&&v%2!==0);td.classList.toggle('multiple',state.multiple>0&&v%state.multiple===0);if(!state.practice)td.classList.remove('revealed')})}
  function setDrawer(open){if(open)setEquationDrawer(false);drawer.classList.toggle('open',open);drawer.setAttribute('aria-hidden',String(!open));$('drawerButton').setAttribute('aria-expanded',String(open))}
  function setEquationDrawer(open){if(open)setDrawer(false);equationDrawer.classList.toggle('open',open);equationDrawer.setAttribute('aria-hidden',String(!open));$('equationButton').setAttribute('aria-expanded',String(open))}
  $('drawerButton').addEventListener('click',()=>setDrawer(!drawer.classList.contains('open')));$('closeDrawer').addEventListener('click',()=>setDrawer(false));$('equationButton').addEventListener('click',()=>setEquationDrawer(!equationDrawer.classList.contains('open')));$('closeEquationDrawer').addEventListener('click',()=>setEquationDrawer(false));
  ...
  const keypad=$('keypad');
  const keys=[['7'],['8'],['9'],['×','operator'],['4'],['5'],['6'],['÷','operator'],['1'],['2'],['3'],['?','operator'],['0','zero'],['⌫','action'],['=','equals']];
  keys.forEach(([k,cls=''])=>{const b=document.createElement('button');b.type='button';b.textContent=k;b.className=cls;b.setAttribute('aria-label',k==='⌫'?'Backspace':k);b.addEventListener('click',()=>{if(k==='⌫')input.value=input.value.slice(0,-1);else input.value+=k;evaluate();input.focus()});keypad.append(b)});
  document.querySelectorAll('[data-example]').forEach(button=>button.addEventListener('click',()=>{input.value=button.dataset.example||'';evaluate();input.focus()}));
