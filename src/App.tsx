...
type Draft={id?:string;name:string;url:string;image:string;type:'game'|'tool'|'link';enabled:boolean;sortOrder:number}
...
function PublicHub(){
 ...
 const visible=items.filter(i=>i.enabled&&(view==='game'?i.type==='game':i.type!=='game')).sort((a,b)=>a.sortOrder-b.sortOrder)
 return <main className="hub"><header className="hub-header"><h1>Tutor Hub</h1></header><nav className="hub-switch" aria-label="Choose games or tools"><button className={view==='game'?'active':''} onClick={()=>setView('game')}>Games</button><button className={view==='tool'?'active':''} onClick={()=>setView('tool')}>Tools</button></nav><section className="card-grid" aria-label={view==='game'?'Games':'Tools'}>{visible.map(item=><a className="hub-card" key={item.id} href={item.url} target={item.url.startsWith('http')?'_blank':undefined} rel="noreferrer">{item.image?<img src={item.image} alt=""/>:<div className="card-placeholder" aria-hidden="true">{item.name.slice(0,1)}</div>}<span>{item.name}</span></a>)}</section>{!visible.length&&<p className="empty-state">No {view==='game'?'games':'tools'} here yet.</p>}</main>
}
